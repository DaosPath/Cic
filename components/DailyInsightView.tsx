...
export const DailyInsightView: React.FC<DailyInsightViewProps> = ({
  ...
  const dateStr = format(parseISO(log.date), datePattern, { locale: dateLocale });
          ...
          {(log.headache || log.migraine || log.migraineWithAura || log.dizziness || log.brainFog) && (
            ...
            <div className="mb-4 pb-4 border-b border-[var(--border)]">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {log.headache && <span className="px-2 py-1 bg-[var(--brand)]/20 rounded text-xs text-[var(--text)]">{t('painLocationHeadache')}</span>}
          ...
          {(log.backPain || log.pelvicPain || log.muscleTension) && (
            ...
            <div className="mb-4 pb-4 border-b border-[var(--border)]">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {log.backPain && <span className="px-2 py-1 bg-[var(--brand)]/20 rounded text-xs text-[var(--text)]">{t('painLocationBack')}</span>}

                {log.pelvicPain && <span className="px-2 py-1 bg-[var(--brand)]/20 rounded text-xs text-[var(--text)]">{t('pain')} pélvico</span>}
...
function generateDailyInsights(log: DailyLog): string[] {
  ...
  if (log.painLevel !== undefined && log.painLevel >= 7) {
    ...
    if (log.painLocations && log.painLocations.length > 0) {
      ...
      insights.push(`{t('pain')} alto en ${log.painLocations.join(', ')}. Considera aplicar calor local y descansar.`);
