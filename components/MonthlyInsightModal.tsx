...
export const MonthlyInsightModal: React.FC<MonthlyInsightModalProps> = ({ logs, cycles, onClose }) => {
  ...
  const stats = calculateMonthlyStats(monthLogs, cycles);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-brand-surface rounded-[18px] border border-brand-border shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        {/* Header */}
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              ...
              <p className="text-sm text-brand-text-dim">
                Análisis completo del mes • {monthLogs.length} días registrados
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <OverviewCard
              ...
              value={`${monthLogs.length}/30`}
              percentage={(monthLogs.length / 30) * 100}
            />
            <OverviewCard
              icon="😴"
            ...
            <OverviewCard
              ...
              percentage={(stats.activeDays / monthLogs.length) * 100}
            />
            <OverviewCard
              icon="💧"
              label="Hidratación"
                  ...
                  <div key={index} className="bg-brand-surface rounded-lg p-3">
                    ...
                    <div className="w-full bg-brand-surface-2 rounded-full h-1 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${(symptom.count / monthLogs.length) * 100}%` }}
                      />
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-gradient-to-br from-brand-primary/10 to-brand-accent/10 border border-brand-primary/20 rounded-xl p-5">
            ...
            <div className="space-y-3">
              {generateMonthlyInsights(stats, monthLogs.length).map((insight, index) => (
