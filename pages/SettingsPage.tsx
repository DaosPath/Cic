...
export const SettingsPage: React.FC = () => {
    ...
    const handleFillLast7Days = async () => {
        try {
            ...
            await refreshData();
            alert(t('devGeneratedLogs', { count: newLogs.length, range: 7 }));
        } catch (error) {
            console.error('Error filling last 7 days:', error);
            alert(t('devGenerateError'));
        }
    };
    ...
    const handleFillLast30Days = async () => {
        try {
            ...
            await refreshData();
            alert(t('devGeneratedLogs', { count: newLogs.length, range: 30 }));
        } catch (error) {
    ...
    const handleGenerateTodayLog = async () => {
        try {
            ...
            await refreshData();
            alert(t('devGeneratedToday'));
        } catch (error) {
            console.error('Error generating today log:', error);
            alert(t('devTodayError'));
        }
    };
    ...
    const handleFillMissingDays = async () => {
        try {
            ...
            await refreshData();
            alert(t('devFilledMissingDays', { count: newLogs.length }));
        } catch (error) {
            console.error('Error filling missing days:', error);
            alert(t('devFillMissingError'));
        }
    };
            ...
            {showToast && (
                <div className="fixed top-4 right-4 z-50 animate-in slide-in-from-top duration-300">
                        <div className="bg-brand-surface border border-brand-primary/30 rounded-[18px] px-6 py-4 shadow-[0_4px_16px_rgba(0,0,0,0.3)] flex items-center gap-3">
                            ...
                            <span className="text-brand-text font-medium">{t('settingsSaved')}</span>
                    </div>
                </div>
            )}
